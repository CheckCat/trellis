## Task
- id: 012
- title: Покрыть тестами ядро backend: валидацию контент-пакета, согласование прогресса по стабильным id, изоляцию sandbox-роли, контракт check-запроса и round-trip экспорта/импорта.
- level: 6
- service: backend
- service_path: services/backend
- role: test-writer
- files: services/backend/tests/courses.test.ts, services/backend/tests/progress.test.ts, services/backend/tests/sandbox.test.ts, services/backend/tests/transfer.test.ts
- depends_on: 009, 010

...
## Commits (5504fba8cfb130ae1d194dd7b108925d5769a353..HEAD)


## Diffstat (5504fba8cfb130ae1d194dd7b108925d5769a353 -> working tree)

 .mvp/ledger.md | 3 +++
 1 file changed, 3 insertions(+)

## Diff (5504fba8cfb130ae1d194dd7b108925d5769a353 -> working tree, tracked files, staged + unstaged)

```diff
diff --git a/.mvp/ledger.md b/.mvp/ledger.md
index 41c9519..4345cc6 100644
--- a/.mvp/ledger.md
+++ b/.mvp/ledger.md
@@ -27,3 +27,6 @@ Task 014: complete (31e2e2876d579e40265a4417e62170f356968294)
...
```
# Windows launcher scripts must arrive on the user's machine with CRLF line
# endings regardless of the developer's platform or core.autocrlf setting:
# cmd.exe is documented to require CRLF in .bat files (LF-only batch files
# misbehave around multi-line blocks in particular), and a .ps1 checked out
# with LF looks broken in Notepad, which is what a non-developer user opens
# it with. Repository storage stays LF (the `text` attribute normalizes on
# commit) — only the checkout is CRLF.
*.bat text eol=crlf
*.ps1 text eol=crlf
```
...
| Что | Зачем |
|---|---|
| Windows 10 или 11, 64-бит | целевая платформа |
...
После установки Docker Desktop перезагрузите компьютер и убедитесь, что в окне
Docker Desktop написано **Engine running**. Дальше Docker можно не открывать —
скрипт запустит его сам, если он не запущен.
...
`start.bat` — это просто обёртка над `start.ps1`, которая подбирает
...
| Запустить снова | тот же `start.bat` |
...
| `Ядро не может войти в базу: пароль в .env не совпадает…` | пароль поменяли после первого запуска | `.\start.ps1 -SyncPasswords` (раздел 5) |
...
| Окно консоли мелькнуло и закрылось | запускали `.ps1` двойным кликом | запускать надо `start.bat` — он держит окно открытым |
| В браузере «Связи с ядром нет» | интерфейс работает, ядро — нет | запустить `start.bat` ещё раз: он дождётся ядра и объяснит причину, если ядро не поднимается |
...
После сброса следующий `start.bat` поднимет платформу с нуля и заново создаст
роли базы с паролями из текущего `.env`.

## 8. Для разработчиков (и не-Windows)

`start.bat`/`start.ps1` — пользовательская упаковка, для разработки они не
нужны. Стек поднимается напрямую:

...
- Курс не должен употреблять термин раньше урока, который его объясняет, — и это касается не только SQL: «сезонность» в первом модуле ничем не лучше `WHERE` до урока про `WHERE`. Словарь курса (`skills.yaml`, секция `terms`) связывает слово с вводящим его уроком, а `grants_sql` у термина — ещё и с конструкцией языка; `course:lint` читает тексты уроков и эталоны заданий и держит порядок (`term-used-before-introduced`, `practice-uses-untaught-sql`). Что линт увидеть не может — опору на понятие без называния его, — остаётся человеку на приёмке.

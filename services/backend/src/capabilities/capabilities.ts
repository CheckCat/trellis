...
const CAPABILITIES_DOCUMENT: EngineCapabilities = {
  ...
  quiz: {
    summary:
      "Один вопрос с вариантами ответа. Обычный квиз — ровно один верный вариант, клик по варианту " +
      "и есть ответ. multiple: true — мультивыбор: ученик отмечает НАБОР вариантов, засчитывается " +
      "точное совпадение с набором верных.",
    manifestFields: [
      { name: "question", valueType: "string", required: true, summary: "Текст вопроса." },
      {
        ...
        default: false,
        summary:
          "Мультивыборный ли квиз. true — чекбоксы и проверка всего набора разом; ответ движка несёт " +
          "вердикт по каждому ОТМЕЧЕННОМУ варианту (и никогда — по неотмеченным).",
      },
      {
        name: "options",
        valueType: "object[]",
        ...
        fields: [
          ...
          { name: "text", valueType: "string", required: true, summary: "Текст варианта." },
          {
            ...
            default: false,
            summary:
              "Верный ли вариант. Без multiple: true — ровно один вариант в квизе несёт correct: true; " +
              "с multiple: true — хотя бы один верный и хотя бы один неверный.",
          },
          {
            name: "explanation",
            valueType: "string",
    ...
    rules: [
      ...
      "Без multiple: true — ровно один вариант с correct: true; несколько верных без явного multiple: true — ошибка валидации, а не молчаливое переключение типа.",
      "С multiple: true — хотя бы один верный и хотя бы один неверный вариант; зачёт только при точном совпадении отмеченного набора с верным.",
      "У каждого неверного варианта обязательно explanation.",
      "Ни correct, ни explanation не покидают backend до ответа ученика: клиент получает только id, text и multiple.",
      "Ответ на мультивыборный квиз несёт вердикт только по отмеченным вариантам; про неотмеченные движок не говорит ничего (в том числе сколько верных не хватает).",

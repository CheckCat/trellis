...
const POSTGRES_SANDBOX: SandboxTypeCapability = {
  ...
  seed:
    "sandboxes[].seed — список путей к .sql-файлам, выполняются по порядку при подключении курса и при " +
    "каждом сбросе песочницы; вместе они должны пересоздавать всё нужное курсу с нуля (DDL + данные).",
  limits: {
